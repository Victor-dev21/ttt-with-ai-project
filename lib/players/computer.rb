...
class Computer < Player
	...
	CORNERS = [1, 3, 9, 7]
		...
		#return strings for the tests to pass
		def move(board)
		if(board.cells[4] == " ")
		return final_cell_move =	"5"
		else
			if(attack_against_opponent(board) != nil)# we havent found an attacking move
				 return final_cell_move = attack_against_opponent(board)
			elsif(defend_against_opponent(board) !=nil) # we dont need to defend
				 return final_cell_move = defend_against_opponent(board)
			elsif(find_empty_corner(board) != nil)
				 return final_cell_move = find_empty_corner(board) # find an empty corner
